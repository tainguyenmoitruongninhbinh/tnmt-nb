...
const DisplayOperatingStatus = ({ data }: any) => {
  ...
  if (
    data.IsError ||
    data.thongso?.hThuongLuu < data.hThuongLuuTT ||
    data.thongso?.qmaxNM < data.qMaxTT ||
    data.thongso?.qXaTran < data.qXaTranTT ||
    data.thongso?.qtt < data.qMinTT
  ) {
  ...
  return <Box>{licenseStatusComponent}</Box>
}
